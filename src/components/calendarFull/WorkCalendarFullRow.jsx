...
const WorkCalendarFullRow = ({
  ...
  return (
    <>
      {lastPostRow && (
        <tr class="last-post-row" style={{ ...(!(showHiddenList ? true : !hiddenEmployeeList?.find((hiddenEmployeeItem) => hiddenEmployeeItem == item.userId)) && { display: 'none' }) }}>
      ...
      <tr className={clsx(lastPostRow && 'last-post-row')} style={{ ...(!(showHiddenList ? true : !hiddenEmployeeList?.find((hiddenEmployeeItem) => hiddenEmployeeItem == item.userId)) && { display: 'none' }) }}>
        <td
          onContextMenu={() => {
            if (isAccessEdit) {
      ...
      {lastIndex - 1 == index && (
        <tr class="last-post-row last-post-row-top" style={{ ...(!(showHiddenList ? true : !hiddenEmployeeList?.find((hiddenEmployeeItem) => hiddenEmployeeItem == item.userId)) && { display: 'none' }) }}>
          <td class="work-calendar-full-cell-small-wrap work-calendar-full-cell-bold" colSpan={2} style={{ position: 'sticky', left: '0px', zIndex: 2, backgroundColor: '#fff' }}>
            Кол-во в смену
          </td>
